...
## Figure 5: Time series exploration

This figure shows dissolved oxygen, temperature, and salinity over time. Panels separate the water quality variables and elevation-code measurement positions, and color shows site. Very high salinity outliers are removed from this exploratory figure so the main salinity pattern is easier to see.

```{r}
#| label: figure-5-time-series
#| fig-width: 12
#| fig-height: 9

# reshape DO, temperature, and salinity into one column for plotting
time_series_data <- do_filtered |> 
  select(
    date,
    site_full,
    elevation_code,
  ...
  pivot_longer(
    cols = c(do_mg_l, temperature_c, salinity_ppt),
    names_to = "variable",
    values_to = "value"
  ) |> 
  filter(!is.na(value)) |> 
  mutate(
    variable = case_when(
      variable == "do_mg_l" ~ "Dissolved oxygen (mg/L)",
      variable == "temperature_c" ~ "Water temperature (°C)",
      variable == "salinity_ppt" ~ "Salinity (ppt)"
    ),
    elevation_code = fct_relevel(
      elevation_code,
      "0", "1", "2"
  ...
  # remove only extreme salinity values for plotting
  filter(
    variable != "Salinity (ppt)" | value <= 50
  )
...
figure_5 <- ggplot(
  ...
  labs(
    title = "Water Quality Variables Over Time by Elevation Code",
    x = "Date",
    y = "Measured value",
    color = "Site"
  ) +
...
This figure is exploratory. It shows how dissolved oxygen, temperature, and salinity are distributed over time across sites and elevation-code positions. Extreme salinity values are removed from the plot so the main salinity pattern is easier to view.
